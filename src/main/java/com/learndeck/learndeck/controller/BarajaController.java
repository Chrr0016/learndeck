...
/**
 * Gestiona las operaciones CRUD de barajas.
 * Tiene dos tipos de endpoints:
 * Endpoints normales que devuelven vistas Thymeleaf (redirecciones)
 * Endpoints AJAX (@ResponseBody) que devuelven JSON para el JavaScript del
 * frontend
 */
@Controller
...
public class BarajaController {
    ...
    // Todos los endpoints lo usan como primera comprobación — si devuelve
    // null es que no hay sesión activa y se redirige a /login.
    ...
    public ResponseEntity<Map<String, Object>> tarjetasJson(@PathVariable Long id,
        ...
        Optional<Baraja> baraja = barajaService.obtenerPorId(id);
        // Verificamos que la baraja pertenece al usuario en sesión antes de
        // devolver sus datos. Sin esta comprobación cualquier usuario podría
        // ver barajas ajenas conociendo el id.
        if (baraja.isEmpty() || !baraja.get().getUsuario().getId().equals(usuarioId)) {
